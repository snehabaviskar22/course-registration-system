...
import { studentApi, courseApi } from '../data/api';
import type { EnrolledCourse } from '../data/mockData';
...
export function MyCoursesPage() {
  ...
  // Load the user's enrolled and waitlisted courses
  const loadCourses = () => {
    if (!user) return;
    studentApi.getMyCourses(user.id)
      .then((data) => setEnrollments(data))
      .catch((err) => showToast(err instanceof Error ? err.message : 'Failed to load courses', 'error'))
      .finally(() => setLoading(false));
  };

  useEffect(loadCourses, [user, showToast]);
  ...
  const handleDrop = async () => {
    if (!dropTarget || !user) return;
    setDropLoading(true);
    try {
      const res = await courseApi.drop(user.id, dropTarget.id);
  ...
  return (
    <DashboardLayout variant="student">
      ...
      <ConfirmDialog
        ...
        confirmLabel={dropLoading ? 'Dropping...' : 'Yes, Drop Course'}
        variant="danger"
      />
    </DashboardLayout>
  );
