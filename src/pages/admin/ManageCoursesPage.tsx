...
import { getCourseStatus, getSeatsLeft, type Course } from '../../data/mockData';
...
export function ManageCoursesPage() {
  ...
  const handleDelete = async () => {
    ...
    try {
      await courseApi.remove(deleteTarget.id);
  ...
  return (
    <DashboardLayout variant="admin">
        ...
        <Card className="p-4 mb-6">
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="flex-1">
              <SearchBar value={search} onChange={setSearch} placeholder="Search courses..." suggestions={courseList.map((c) => c.name)} />
            </div>
            <Select
              value={dept}
      ...
      <ConfirmDialog
        ...
        confirmLabel={deleteLoading ? 'Deleting...' : 'Delete Course'}
        variant="danger"
      />
