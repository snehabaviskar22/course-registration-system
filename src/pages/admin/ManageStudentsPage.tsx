...
import { adminApi, departmentApi } from '../../data/api';
...
export function ManageStudentsPage() {
  ...
  const handleDeactivate = async () => {
    ...
    try {
      ...
      await adminApi.updateStudentStatus(deactivateTarget.id, newStatus);
      showToast(`${deactivateTarget.name} has been ${newStatus === 'Active' ? 'activated' : 'deactivated'}`, 'info');
      loadStudents();
    } catch (err) {
  ...
  return (
    <DashboardLayout variant="admin">
        ...
        <Card className="p-4 mb-6">
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="flex-1">
              <SearchBar value={search} onChange={setSearch} placeholder="Search by name, email, or ID..." suggestions={studentList.map((s) => s.name)} />
            </div>
            <Select
              value={dept}
      ...
      <ConfirmDialog
        ...
        confirmLabel={actionLoading ? 'Processing...' : deactivateTarget?.status === 'Active' ? 'Deactivate' : 'Activate'}
        variant="danger"
      />
    </DashboardLayout>
  );
