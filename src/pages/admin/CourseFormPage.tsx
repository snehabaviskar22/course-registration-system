...
export function CourseFormPage({ mode, courseId }: { mode: 'create' | 'edit'; courseId?: string }) {
  ...
  // Fetch departments and course data if in edit mode
  useEffect(() => {
    departmentApi.getAll().then((d) => setDepartments(d.map((dep) => dep.name))).catch(() => {});
    if (mode === 'edit' && courseId) {
      courseApi.getById(courseId).then((c) => {
        setForm({
          name: c.name,
          code: c.code,
          ...
          waitlistCapacity: String(c.waitlistCapacity),
          deadline: c.deadline,
