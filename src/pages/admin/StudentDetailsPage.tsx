...
import { adminApi, studentApi } from '../../data/api';
import type { Student, EnrolledCourse } from '../../data/mockData';
...
export function StudentDetailsPage({ id }: { id: string }) {
  ...
  useEffect(() => {
    Promise.all([adminApi.getStudent(id), studentApi.getMyCourses(id)])
      .then(([s, c]) => {
        setStudent(s);
        setCourses(c);
      })
  ...
  if (!student) {
    return (
      <DashboardLayout variant="admin">
        <Card><EmptyState title="Student not found" /></Card>
      </DashboardLayout>
    );
  }

  const enrolled = courses.filter((c) => c.status === 'ENROLLED');
  const waitlisted = courses.filter((c) => c.status === 'WAITLISTED');

  const timeline = courses.slice(0, 4).map((c) => ({
    message: `${c.status === 'ENROLLED' ? 'Registered for' : 'Added to waitlist for'} ${c.name}`,
    time: c.enrollmentDate ? new Date(c.enrollmentDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : '',
    type: c.status === 'ENROLLED' ? 'register' : 'waitlist',
  }));
          ...
          <div className="flex flex-col sm:flex-row items-start gap-5">
            ...
            <div className="flex-1">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                ...
                <InfoItem icon={Mail} label="Joined" value={student.joinedDate ? new Date(student.joinedDate).toLocaleDateString('en-US', { month: 'short', year: 'numeric' }) : '—'} />
              </div>
            </div>
          </div>
        </Card>
